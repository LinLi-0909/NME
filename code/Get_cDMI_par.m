function [out] = Get_cDMI_par(GEM,regulators,gene_names,nz,p,l,timeseries,ns,ne)
...
if timeseries>0
    GEM = GEM(:,2:end);
    fprintf('Compute the causaity of timeseries data...')
else
    fprintf('Compute the casusality of non-timeseries data...')
end

[m,n] = size(GEM);
dc_GEM = mat2cell(GEM,m,ones(1,n));
...
cDMI = zeros(length(regulators),ne-ns+1);
 for k = 1:nz
